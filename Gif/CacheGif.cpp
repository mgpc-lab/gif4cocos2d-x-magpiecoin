...
{
	const char* frame = (m_gif_fullpath + "_" + std::to_string(index)).c_str();   //  std::string(CCString::createWithFormat("%s_%d", m_gif_fullpath.c_str(), index)->getCString();
	return frame;
}
...
void CacheGif::updateGif(uint32_t delta)
{
	...
	for (int i = 0; i < m_frameData.size(); i++)
	{
		...
		if (dur >= m_progress)// If the progress bigger than sum of pre frame duration，change new CCSpriteFrame to display
		{
			...
			{
				this->setSpriteFrame(m_frameData[i]->getSpriteFrame());
			}
